...
public class StribogConstants {
    ...
    /**
     * Класс, содержащий все статические константы, необходимые для реализации
     * хэш-функции Стрибог (ГОСТ Р 34.11-2018):
     * - Таблица замен S
    ...
    /**
     * Преобразует число байт в массив int[64] для использования в алгоритме Стрибог.
     * @param num Количество байт (обычно 512/8 = 64).
     * @return Массив длины 64, содержащий num в младших 8 байтах.
     */
    public int[] numByte(long num) {
        var result = new int[64];
        long numBits = num * 8; // Длина в битах

        for (int i = 0; i < 8; i++) {
            // Записываем младшие 8 байт (512 бит) длины
            result[63 - i] = (int) (numBits & 0xFF);
            numBits >>>= 8;
        }
        return result;
    }
